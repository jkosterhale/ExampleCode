function KMVPA_run_classification(study, task, subjectlist, runsincluded, nvoxelarg,  varargin)

%e.g. KMVPA_run_classification('KMVPA', 'KMVPA', makeIDs('aKMVPA', [1:9 11 13:17 19:20 ]), '1to5fullDur_offset0',100)
% codified from on princton mvpa toolbox documentation (see tutorial) and
% template from AES (April 2013) 
...
% prep_for_mvpaptb)
% main function called is mvpaptb_classify
...
folderprefix=['06052014_adults_allrois_' num2str(nvoxelarg) 'v_fullDur_offset0_'];
masksubset={};
discsubset={};
studydir=[rootdir study '/'];
saveOutputScript='/mindhive/saxelab/scripts/jkh_scripts/mvpaptb/Core/printandsave_mvpaptb';
santityImages=0; %set this to 1 if you want to be a space gobbler (and get some sanity check figs). save this for when debugging a new analysis/feature
checkbeforedelete=0;
...
other_args.hemodynamic_delay=4; %in seconds 6 would also be reasonable...
...
other_args.fixednum=nvoxelarg; %matters only if fixed=1this specifies the number of voxels to take if using fixed rank-based feature selection
...
other_args.voxelthreshold=20; %matters only if fixed=0... freak out if xval fold has fewer than this many voxels
...
other_args.hpval = 128;  
other_args.detrend=1; %default=1
...
other_args.notes='First attempt, all default settings'; % write yourself some  notes about this analysis
    ...
    multi=0;
        ...
        multi=1;
            ...
            multi=0;
                ...
                multi=1;
            else if strcmp(other_args.classifier, 'libsvm') %see train_libsvm.m in scripts/mvpaptb... uses libsvm library stored in scripts/svm, downloaded from here: http://www.csie.ntu.edu.tw/~cjlin/libsvm/
                    ...
                    class_args.svm_type = 0; %svm_type : set type of SVM (default 0): 0 -- C-SVC; 1 -- nu-SVC; 2 -- one-class SVM; 3 -- epsilon-SVR; 4 -- nu-SVR
                        ...
                        %see train_liblinear for  notes about this, and key for specifying
                        %desired classifier
                    end
                end
            end
...
for s=1:numSubj
    ...
    if other_args.globalnorm
        ...
        subj = load_analyze_pattern_JKH(subj,other_args.bolds,mask_description, boldnames, temp_args);
    ...
    foldername=[folderprefix 'crossruns_' task '_' runsincluded '_' class '_' other_args.imagetype '_' boc '_' won '_' hrfshift '_' fs '_' fsf '_' aon '_' zs '_' hp '_' dt '_' gn '_' s4c]
    ...
    for m=minmask:maxmask
        %for m=1
        percentcomplete=m/numMasks*100; %how far along?
        ...
        if dothismask
            ...
            % now, read and set up the actual data, keeping only the voxels active in the
            % mask (see above)
            subj = load_analyze_pattern_JKH(subj,other_args.bolds,mask_description, boldnames, other_args);
            ...
            if multi
                ...
                startd=2; % else skip the first and use binary only (NOTE: this logic assumes that only first discrimination (all conditions) is multiclass.)
            end
...
function [subj] = load_analyze_pattern_JKH(subj,new_patname,maskname,filenames, other_args, varargin)
% minor tweaks
...
for h = 1:nFiles
    if mod(h,400)==0 %% added by  because printout was annoying
        fprintf('\t%i',h);
    end
